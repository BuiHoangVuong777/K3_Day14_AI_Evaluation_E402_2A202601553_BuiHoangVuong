...
def rerank_by_overlap(contexts: list[str], query: str) -> list[str]:
    ...
    """
    return sorted(
        contexts, key=lambda c: len(_tokenize(c) & _tokenize(query)), reverse=True
    )
...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = (
            ...
            'Respond as JSON, e.g. {"criterion_name": 0.8, ...}.'
        )
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        per_entry_avg = [_avg(entry) for entry in scores_batch]
        overall_avg = (
            sum(per_entry_avg) / len(per_entry_avg) if per_entry_avg else 0.0
        )

        # Position bias: alternate entries are assumed to represent the same
        # question judged with two response orders (first vs. second).
        positional_bias = False
        if len(per_entry_avg) >= 2:
            first_position = per_entry_avg[0::2]
            second_position = per_entry_avg[1::2]
            if first_position and second_position:
                first_avg = sum(first_position) / len(first_position)
                second_avg = sum(second_position) / len(second_position)
...
class FailureAnalyzer:
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for index, failure in enumerate(failures):
            ...
            suggested_fix = suggestions[index] if index < len(suggestions) else "Review manually"
            rows.append(
    ...
    def generate_improvement_suggestions(
        ...
        if categories.get("hallucination"):
            suggestions.append(
                "Implement a hallucination checker to filter claims unsupported "
        ...
        if categories.get("off_topic"):
            suggestions.append(
                "Add intent detection safeguards to prevent off-topic responses"
            )
        ...
        fallback_suggestions = [
            ...
            "Expand the golden dataset with cases similar to these failures",
            "Review retrieval ranking to surface the most relevant chunks earlier",
        ]
